...
def log_spectrograms(spectrogram, reconstruction, tag="MDVAE_spectrograms"):
...
def get_class_labels(descriptor, true_labels, class_mappings='class_mappings.json'):
    ...
    rev_map = {v: k for k, v in class_mappings[dict_key].items()}
    return [rev_map[i] for i in range(len(rev_map))]

def plot_confusion_matrix(cm, descriptor='', true_labels=None, output_path='./confusion_matrix'):
    '''Plot and save confusion matrix as PNG and log to wandb'''
    class_labels = get_class_labels(descriptor, true_labels)
...
def plot_latent_space(latents, true_labels, descriptor, output_path):
    ...
    os.makedirs(output_path, exist_ok=True)
    save_path = os.path.join(output_path, f"latent_space_{descriptor}.png")
